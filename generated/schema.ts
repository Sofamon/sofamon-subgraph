...
export class setLevel extends Entity {
  constructor(id: Bytes) {
    super();
    this.set("id", Value.fromBytes(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save setLevel entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.BYTES,
        `Entities of type setLevel must have an ID of type Bytes but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("setLevel", id.toBytes().toHexString(), this);
    }
  }

  static load(id: Bytes): setLevel | null {
    return changetype<setLevel | null>(store.get("setLevel", id.toHexString()));
  }
